...
from cyberbattle.simulation.model_pivot import MachineStatus, PrivilegeLevel, PropertyName, VulnerabilityID, VulnerabilityType
from . import model_pivot as model
...
#########################
            # DRL Added #
            f"[!] Edge_Annotation for Source Node '{source_node_id}' to Target Node '{target_node_id}' : '{edge_annotation}'"
######################### 

            if 'kind' in edge_annotation:
                new_annotation = EdgeAnnotation(max(edge_annotation['kind'].value, new_annotation.value))
            else:
                #print("[!] Failed Here... Source Node {0}, Target {1}".format(source_node_id, target_node_id))
    ...
    def __process_outcome(self,
        ...
        if isinstance(outcome, model.PrivilegeEscalation):
            if outcome.tag in node_info.properties:
                ...
                # Repeated actions penalized here
...
####################
#
# Connects to remote machine with credentials as opposed via an exploit
#
#
####################
